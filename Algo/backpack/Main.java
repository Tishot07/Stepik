...
class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            itemList.add(new Item(scanner.nextLong(), scanner.nextLong()));
        }
        itemList.sort(Comparator.comparing(Item::getCoastPerKilo).reversed());
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (backpack.capacity >= nowItem.weight) {
                result += nowItem.coast;
                backpack.capacity -= nowItem.weight;
            }
            else {
                result += nowItem.coastPerKilo * backpack.capacity;
                backpack.capacity = 0;
            }
        }
...
class Backpack {
    long volume;
    long capacity;
    protected Backpack (long volume) {
        this.volume = volume;
...
class Item {
    ...
    long weight;
    double coastPerKilo;
    protected Item(long coast, long weight) {
        this.coast = coast;
        this.weight = weight;
        this.coastPerKilo = (double) coast / weight;
    }
    protected double getCoastPerKilo() {
        return this.coastPerKilo;
    }
}
 */